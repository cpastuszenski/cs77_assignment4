...
image<vec3f> imageio_read_png3f(const string& filename, bool flipY) {
    ...
    if(error) { WARNING("cannot read png image: %s", filename.c_str()); return image<vec3f>(); }
...
void imageio_write_png(const string& filename, const image<vec3f>& img, bool flipY) {
    ...
    if(error) { WARNING("cannot write png image: %s", filename.c_str()); }
}

void imageio_write_png16(const string& filename, const image<vec3f>& img, bool flipY) {
    vector<unsigned short> img_png16(img.width()*img.height()*4);
    for(int x = 0; x < img.width(); x++ ) {
        for( int y = 0; y < img.height(); y++ ) {
            int i_img = y * img.width() + x;
            int i_png = ( ( flipY ? (img.height()-1-y) : y ) * img.width() + x ) * 4;
            img_png16[i_png+0] = (unsigned short)clamp(img.data()[i_img].x * 255, 0.0f, 65535.0f);
            img_png16[i_png+1] = (unsigned short)clamp(img.data()[i_img].y * 255, 0.0f, 65535.0f);
            img_png16[i_png+2] = (unsigned short)clamp(img.data()[i_img].z * 255, 0.0f, 65535.0f);
            img_png16[i_png+3] = 65535;
        }
    }
    // flip endianess
    vector<unsigned char> img_png(img_png16.size()*2);
    for(auto i : range(img_png16.size())) {
        img_png[i*2+0] = (unsigned char)(img_png16[i] << 8);
        img_png[i*2+1] = (unsigned char)(img_png16[i] & 255);
    }
    unsigned error = lodepng::encode(filename, img_png, img.width(), img.height(), LCT_RGBA);
    if(error) { WARNING("cannot write png image: %s", filename.c_str()); }
}

void imageio_write_auto(const string& filename, const image<vec3f>& img, bool flipY) {
    if(string_endswith(filename, "png")) imageio_write_png(filename, img, flipY);
    else if(string_endswith(filename, "pfm")) imageio_write_pfm(filename, img, flipY);
    else if(string_endswith(filename, "ppm")) imageio_write_ppm(filename, img, flipY);
    else ERROR("image extension not supported");
}

image3f imageio_read_auto3f(const string& filename, bool flipY) {
    if(string_endswith(filename,"png")) return imageio_read_png3f(filename, flipY);
    else if(string_endswith(filename,"pfm") or string_endswith(filename,"ppm")) return imageio_read_pnm3f(filename, flipY);
    else ERROR("unsupported image extension");
    return image3f();
}
